...
struct AVLTree {
  ...
  // 辅助函数: 验证树的AVL属性
  bool isAVL(AVLNode* node, int& height) const {
    if (node == nullptr) {
      height = -1;
      return true;
    }
    int leftHeight, rightHeight;
    if (!isAVL(node->child[LEFT], leftHeight) ||
        !isAVL(node->child[RIGHT], rightHeight)) {
      return false;
    }
    if (std::abs(leftHeight - rightHeight) > 1) {
      std::cerr << "  [FAIL] Node " << node->elem
                << " is unbalanced! BF: " << (leftHeight - rightHeight)
                << std::endl;
      return false;
    }
  ...
  // 辅助函数: 验证BST属性
  bool isBST(AVLNode* node, const DataType* min_val,
             const DataType* max_val) const {
    if (node == nullptr) return true;
    if ((min_val && node->elem <= *min_val) ||
        (max_val && node->elem >= *max_val)) {
      std::cerr << "  [FAIL] Node " << node->elem << " violates BST property."
                << std::endl;
      return false;
    }
    return isBST(node->child[LEFT], min_val, &node->elem) &&
           isBST(node->child[RIGHT], &node->elem, max_val);
  }
  ...
  bool checkIntegrity() const {
    ...
    if (root->parent != nullptr) {
      std::cerr << "  [FAIL] Root's parent is not null!" << std::endl;
      return false;
    }
    int height;
    if (!isBST(root, nullptr, nullptr)) return false;
    if (!isAVL(root, height)) return false;
